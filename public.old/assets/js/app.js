...
Pylon.on("newt-flash", function(where) {
  var i;
  i = Math.floor(30 * Math.random());
  console.log(i, `draft/khmer/consonants/audio/c${i + 1}.mp3`);
  return $(where).html(T.render(function() {
    var khmerChar;
    khmerChar = String.fromCharCode(0x1780 + i);
    return T.div(".card", function() {
      T.div({
        style: "line-height:18rem;font-size:15rem"
      }, khmerChar);
      return T.div(".card-block", function() {
        return T.div(".media", function() {
          return T.tag("figure", ".d-flex.mr-3", function() {
            return T.div(".media-body", function() {
              return T.tag("audio", {
                controls: "controls",
                src: `draft/khmer/consonants/audio/c${i + 1}.mp3`
              }, "broken browser");
            });
          });
        });
